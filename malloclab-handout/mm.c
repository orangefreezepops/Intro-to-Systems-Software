...
static BlockInfo* searchFreeList(size_t reqSize) {
	BlockInfo* freeBlock;
	//BlockInfo* bestYet; //best yet variable holds data for the best block that isn't the exact size requested
	freeBlock = FREE_LIST_HEAD;
	//bestYet = FREE_LIST_HEAD;
	
	// YOUR CODE HERE!
	//loop over the freeBlock object while youre not at the end of the freespace
	//if free space is found and is the same size as the space requested, return it
	//if we loop thru the entirety of the heap and there is no block suitable for the
	...
	while (freeBlock != NULL) {
		if (SIZE(freeBlock->sizeAndTags) == reqSize){
			return freeBlock;
		//update and keep track of the bestYet block size 
		}
		if ((SIZE(freeBlock->sizeAndTags) > reqSize) && (SIZE(freeBlock->sizeAndTags) < SIZE(bestYet->sizeAndTags))){
			bestYet = freeBlock;
		} 
		
		if (freeBlock->next == NULL){
			//if the next block will get you to the end of the free list 
			// and there werent any matches of the exact size requested, return the best one found
			return bestYet;
		}
	...
	*/
	...
	//Attempt at best fit wasn't working, so default to 
...
void* mm_malloc(size_t size) {
	...
	reqSpaceBlock = searchFreeList(reqSize);
	
	//if searching the free list doesn't return an adress with the size requested
	if (reqSpaceBlock == NULL){
		//request more space
		requestMoreSpace(reqSize);
		//secondary BlockInfo object is now the holder of the new address
		reqSpaceBlock = searchFreeList(reqSize);
		precedingBlockUseTag = TAG_PRECEDING_USED;
	} else {
		precedingBlockUseTag = reqSpaceBlock->sizeAndTags & TAG_PRECEDING_USED;
	}
	blockSize = SIZE(reqSpaceBlock->sizeAndTags);
	
	//then remove the block from the free list
	removeFreeBlock(reqSpaceBlock);	
	
	
	if (MIN_BLOCK_SIZE <= (blockSize - reqSize)){
		//set ptrFreeBlock to the newly allocated space + requested size
		ptrFreeBlock = UNSCALED_POINTER_ADD(reqSpaceBlock, reqSize);
		
		//set TAG_PRECEDING_USED in header and footer of the new block 
		ptrFreeBlock->sizeAndTags = (blockSize - reqSize) | TAG_PRECEDING_USED;
		*((size_t*)UNSCALED_POINTER_ADD(ptrFreeBlock, (blockSize - reqSize - WORD_SIZE))) = (blockSize - reqSize) | TAG_PRECEDING_USED;
		
		insertFreeBlock(ptrFreeBlock);
		coalesceFreeBlock(ptrFreeBlock);
		reqSpaceBlock->sizeAndTags = TAG_USED | precedingBlockUseTag | reqSize;
	} else {
		//otherwise set header to show its the entire block size
		reqSpaceBlock->sizeAndTags = TAG_USED | precedingBlockUseTag | blockSize;
		
		//make a new BlockInfo item with the allocated space plus the size of the block
		BlockInfo* nextB = UNSCALED_POINTER_ADD(reqSpaceBlock, blockSize);
		size_t nextSize = SIZE(nextB->sizeAndTags);
		size_t nextUsed = nextB->sizeAndTags & TAG_USED;
		
		*((size_t*)UNSCALED_POINTER_ADD(nextB, nextSize - WORD_SIZE)) = (TAG_PRECEDING_USED | nextUsed | nextSize);
		
		nextB->sizeAndTags = TAG_PRECEDING_USED | nextUsed | nextSize;
		
	}
	return UNSCALED_POINTER_ADD(reqSpaceBlock, WORD_SIZE);
}
...
void mm_free(void *ptr) {
	...
	//make sure the block isnt already freed
	if (((blockInfo->sizeAndTags) && TAG_USED) == 0){
		//cant do nuffin
		return;
	}
	
	//otherwise mark the freed block and the following as unused
	...
	//set the footer of it and the following
	*((size_t*)UNSCALED_POINTER_ADD(blockInfo, SIZE(blockInfo->sizeAndTags) - WORD_SIZE)) = 
	SIZE(blockInfo->sizeAndTags) | (blockInfo->sizeAndTags & TAG_USED);
	
	*((size_t*)UNSCALED_POINTER_ADD(followingBlock, SIZE(followingBlock->sizeAndTags) - WORD_SIZE)) = 
	SIZE(followingBlock->sizeAndTags) | (followingBlock->sizeAndTags & TAG_USED);
...
int mm_check() {
	//not really sure what to do here, so...
	examine_heap(); // i guess?
	return 0;
}
