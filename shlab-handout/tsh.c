...
void eval(char *cmdline) 
{
	...
	if (!builtin_cmd(argv)){
		...
		//Block signals and save previous blocked set
		sigprocmask(SIG_BLOCK, &mask, NULL);
		if ((pid = fork()) == 0){ //run child
...
void do_bgfg(char **argv) 
{
	...
	if (argv[1] == NULL){
		printf("%s requires either a PID or %%jobid argument\n", argv[0]);
		return;
	}

	if (!isdigit(argv[1][0]) && argv[1][0] != '%'){
		printf("%s argument must be either a PID or %%jobid\n", argv[0]);
		return;
	}
